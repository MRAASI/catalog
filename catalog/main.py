...
rups_cat = session.query(LanguageName).all()
...
def showLogin():
    ...
    rups_cat = session.query(LanguageName).all()
    rupes = session.query(PaperName).all()
    return render_template('login.html',
                           STATE=state, rups_cat=rups_cat, rupes=rupes)
    # return render_template('myhome.html', STATE=state
    # rups_cat=rups_cat,rupes=rupes)
...
def home():
    rups_cat = session.query(LanguageName).all()
    return render_template('myhome.html', rups_cat=rups_cat)
...
def PaperStore():
    try:
        if login_session['username']:
            ...
            rups_cat = session.query(LanguageName).all()
            rups = session.query(LanguageName).all()
            rupes = session.query(PaperName).all()
            return render_template('myhome.html', rups_cat=rups_cat,
                                   rups=rups, rupes=rupes, uname=name)
    except:
...
@app.route('/PaperStore/<int:rupid>/AllPapers')
def showPapers(rupid):
    rups_cat = session.query(LanguageName).all()
    rups = session.query(LanguageName).filter_by(id=rupid).one()
    rupes = session.query(PaperName).filter_by(languagenameid=rupid).all()
    try:
        if login_session['username']:
            return render_template('showPapers.html', rups_cat=rups_cat,
                                   rups=rups, rupes=rupes,
                                   uname=login_session['username'])
    except:
        return render_template('showPapers.html',
                               rups_cat=rups_cat, rups=rups, rupes=rupes)
...
def addLanguage():
    ...
    if request.method == 'POST':
        ...
        return render_template('addLanguage.html', rups_cat=rups_cat)

########
# Edit Paper Category


@app.route('/PaperStore/<int:rupid>/edit', methods=['POST', 'GET'])
def editPaperCategory(rupid):
    if "username" not in login_session:
        flash("please login first")
        return redirect(url_for("showLogin"))
    editedPaper = session.query(LanguageName).filter_by(id=rupid).one()
    ...
    if request.method == "POST":
        ...
        # rups_cat is global variable we can them in entire application
        return render_template('editPaperCategory.html',
                               rup=editedPaper, rups_cat=rups_cat)

######
# Delete Paper Category


@app.route('/PaperStore/<int:rupid>/delete', methods=['POST', 'GET'])
def deletePaperCategory(rupid):
    if "username" not in login_session:
        flash("please login first")
        return redirect(url_for("showLogin"))
    rup = session.query(LanguageName).filter_by(id=rupid).one()
    creator = getUserInfo(rup.user_id)
    user = getUserInfo(login_session['user_id'])
    # If logged in user != item owner redirect them
    if creator.id != login_session['user_id']:
    ...
    if request.method == "POST":
        ...
        return render_template(
            'deletePaperCategory.html', rup=rup, rups_cat=rups_cat)

######
# Add New Paper Details


@app.route('/PaperStore/addCompany/addPaperDetails/<string:rupname>/add',
           methods=['GET', 'POST'])
def addPaperDetails(rupname):
    if "username" not in login_session:
        flash("please login first")
        return redirect(url_for("showLogin"))
    rups = session.query(LanguageName).filter_by(name=rupname).one()
    creator = getUserInfo(rups.user_id)
    user = getUserInfo(login_session['user_id'])
    if creator.id != login_session['user_id']:
        flash("You can't add new book edition"
              "This is belongs to %s" % creator.name)
        return redirect(url_for('showPapers', rupid=rups.id))
    if request.method == 'POST':
        ...
        return render_template('addPaperDetails.html',
                               rupname=rups.name, rups_cat=rups_cat)

######
# Edit Paper details


@app.route('/PaperStore/<int:rupid>/<string:rupename>/edit',
           methods=['GET', 'POST'])
def editPaper(rupid, rupename):
    if "username" not in login_session:
        flash("please login first")
        return redirect(url_for("showLogin"))
    rup = session.query(LanguageName).filter_by(id=rupid).one()
    paperdetails = session.query(PaperName).filter_by(name=rupename).one()
    ...
    if creator.id != login_session['user_id']:
        flash("You can't edit this book edition"
              "This is belongs to %s" % creator.name)
        return redirect(url_for('showPapers', rupid=rup.id))
    # POST methods
    if request.method == 'POST':
        ...
        return redirect(url_for('showPapers', rupid=rupid))
    else:
        return render_template(
            'editPaper.html', rupid=rupid, paperdetails=paperdetails,
            rups_cat=rups_cat)
...
@app.route('/PaperStore/<int:rupid>/<string:rupename>/delete',
           methods=['GET', 'POST'])
def deletePaper(rupid, rupename):
    if "username" not in login_session:
        flash("please login first")
        return redirect(url_for("showLogin"))
    rup = session.query(LanguageName).filter_by(id=rupid).one()
    ...
    if request.method == "POST":
        ...
        return render_template('deletePaper.html',
                               rupid=rupid, paperdetails=paperdetails,
                               rups_cat=rups_cat)
...
def allPaperssJSON():
    ...
    for c in range(len(category_dict)):
        papers = [i.serialize for i in session.query(
                 PaperName).filter_by(
                     languagenameid=category_dict[c]["id"]).all()]
        if papers:
...
def categoriesJSON():
    papers = session.query(LanguageName).all()
...
def categoryItemsJSON(paper_name):
    paperCategory = session.query(LanguageName).filter_by(
        name=paper_name).one()
    papers = session.query(PaperName).filter_by(
                                            languagename=paperCategory).all()
...
def ItemJSON(paper_name, edition_name):
    ...
    paperEdition = session.query(PaperName).filter_by(
           name=edition_name, languagename=paperCategory).one()
